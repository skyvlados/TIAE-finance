...
class OperationQueryTest < ActiveSupport::TestCase
    ...
    test 'by date start' do     
        assert_equal OperationQuery.new(date_start: '2020-01-05').call.to_a, operations(:food, :relax, :each_goods, :goods)
    end

    test 'by date finish' do     
        assert_equal OperationQuery.new(date_finish: '2020-01-05').call.to_a, operations(:goods, :salary)
    end

    test 'by direction, category, currency, date' do     
        assert_equal OperationQuery.new(direction: 'expenditure', currency: 'RUB', category: Category.find_by(name: 'goods').id, date_start: '2020-01-05', date_finish: '2020-01-05').call.to_a, [operations(:goods)]
    end
end